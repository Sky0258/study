...
fs.readFile(path.join(__dirname,'./file/分隔文件案例/24.缓动动画.html'),'utf-8',function(error,dataStr) {
    ...
    function resolveCSS(dataStr) {
        ...
        // 将结果写入css文件
        fs.writeFile(path.join(__dirname,'./file/分隔文件案例/分隔案例.css'),r2,function(err) {
            if(err) {
    ...
    function resolveScript(dataStr) {
        ...
        // 将结果写入js文件
        fs.writeFile(path.join(__dirname,'./file/分隔文件案例/分隔案例.js'),r2,function(err) {
            if(err) {
    ...
    function resolveHTML(dataStr) {
        ...
        fs.writeFile(path.join(__dirname,'./file/分隔文件案例/分隔案例.html'),r1,function(err) {
            if(err) {
